  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900/50 border border-slate-800 rounded-xl overflow-hidden shadow-sm">
        <div className="p-4 border-b border-slate-800 flex flex-wrap gap-3 bg-black/50">
          <div className="relative flex-1 min-w-[200px]">
            <Search className="absolute left-2.5 top-2.5 h-4 w-4 text-slate-400" />
            <input
              type="text"
              placeholder="Search origin, destination, airline..."
              className="pl-9 pr-4 py-2 border border-slate-800 rounded-lg text-sm w-full outline-none focus:ring-2 focus:ring-indigo-500"
            />
          </div>
          <button className="flex items-center gap-2 px-3 py-2 border border-slate-800 bg-slate-900/50 rounded-lg text-sm font-medium text-slate-600 hover:bg-black">
            <Filter size={16} />
            Filters
          </button>
        </div>
        ...
        {loading ? (
          ...
          <DataTable 
            ...
            columns={[
              ...
              { header: 'Origin', accessor: 'origin', className: 'font-semibold text-slate-300' },
              { header: 'Dest', accessor: 'destination', className: 'font-semibold text-slate-300' },
        ...
        <div className="p-4 border-t border-slate-800 flex items-center justify-between text-sm text-slate-500 bg-black/50">
          <div>Showing 1 to {data.length} of {data.length} entries</div>
          <div className="flex gap-1">
            <button className="px-3 py-1 border border-slate-800 rounded bg-slate-900/50 text-slate-400 cursor-not-allowed">Previous</button>
            <button className="px-3 py-1 border border-slate-800 rounded bg-slate-900/50 text-slate-400 cursor-not-allowed">Next</button>
          </div>
        </div>
      </div>
    </div>
